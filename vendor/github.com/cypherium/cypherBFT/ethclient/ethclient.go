...
// Package ethclient provides a client for the Cypherium RPC API.
package ethclient
...
func (ec *Client) BlockHeadersByNumbers(ctx context.Context, numbers []int64) ([]*types.Header, []int, error) {
	...
	err := ec.c.CallContext(ctx, &raws, "eth_getTxBlockHeadersByNumbers", numbers)
	if err != nil {
...
func (ec *Client) SubscribeLatestTPS(ctx context.Context, ch chan<- uint64) (cypherI.Subscription, error) {
	return ec.c.EthSubscribe(ctx, ch, "latestTPS")
}
...
func (ec *Client) PendingStorageAt(ctx context.Context, account common.Address, key common.Hash) ([]byte, error) {
	var result hexutil.Bytes
	err := ec.c.CallContext(ctx, &result, "eh_getStorageAt", account, key, "pending")
	return result, err
}
