...
type ChainReader interface {
	BlockByHash(ctx context.Context, hash common.Hash, inclTx bool) (*types.Block, int, error)
	BlockByNumber(ctx context.Context, number *big.Int) (*types.Block, int, error)
...
// the Cypherium network.
type SyncProgress struct {
	StartingKeyBlock uint64 // Key Block number where sync began
