...
func newKeyService(s serviceI, eth Backend, config *params.ChainConfig) *keyService {
	keyS := new(keyService)
	keyS.s = s
	keyS.candidatepool = eth.CandidatePool()
	keyS.bc = eth.BlockChain()
	keyS.kbc = eth.KeyBlockChain()
	keyS.engine = eth.Engine()
	keyS.config = config
