...
.PHONY: cypher android ios cypher-cross swarm evm all test clean
...
txtest:
	build/env.sh go run build/ci.go install ./cmd/txtest
	@echo "Done building."
	@echo "Run \"$(GOBIN)/txtest\" to launch txtest."
...
ios:
	...
	@echo "Done building."
	@echo "Import \"$(GOBIN)/Cypher.framework\" to use the library."
...
cypher-linux-arm-6:
	build/env.sh go run build/ci.go xgo -- --go=$(GO) --targets=linux/arm-6 -v ./cmd/cypher
