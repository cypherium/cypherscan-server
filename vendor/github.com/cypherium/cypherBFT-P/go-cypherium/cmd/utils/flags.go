...
import (
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/common/fdlimit"
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/cph/gasprice"
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/metrics"
	"github.com/cypherium/cypherBFT-P/go-cypherium/metrics/influxdb"
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/p2p/netutil"
...
var (
	...
	ReconfigEnabledFlag = cli.BoolFlag{
		Name:  "reconfig",
		Usage: "Enable reconfig",
	}
	...
	BootnodesFlag = cli.StringFlag{
		Name:  "bootnodes",
		Usage: "Comma separated cnode URLs for P2P discovery bootstrap (set v4+v5 instead for light servers)",
		Value: "",
	}
	BootnodesV4Flag = cli.StringFlag{
		Name:  "bootnodesv4",
		Usage: "Comma separated cnode URLs for P2P v4 discovery bootstrap (light server, full nodes)",
		Value: "",
	}
	BootnodesV5Flag = cli.StringFlag{
		Name:  "bootnodesv5",
		Usage: "Comma separated cnode URLs for P2P v5 discovery bootstrap (light server, light nodes)",
		Value: "",
	}
...
// for Cypher and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `cypher account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	//	ethPeers = 0
	//}
	//log.Info("Maximum peer count", "Cypherium", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
func SetLocalTestTestIpSw(ctx *cli.Context, cfg *core.LocalTestIpConfig) {
	if ctx.GlobalIsSet(LocalTestIpConfig.Name) {
		cfg.LocalTestIP = ctx.GlobalString(LocalTestIpConfig.Name)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "cypher.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
// new format.
//
// e.g. cypher account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// cypher --keystore /tmp/mykeystore --lightkdf account new
//
