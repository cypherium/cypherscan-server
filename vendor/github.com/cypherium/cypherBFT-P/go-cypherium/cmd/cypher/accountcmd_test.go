...
func TestAccountListEmpty(t *testing.T) {
	cypher := runCph(t, "account", "list")
	cypher.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	cypher := runCph(t, "account", "list", "--datadir", datadir)
	defer cypher.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	cypher := runCph(t, "account", "new", "--lightkdf")
	defer cypher.ExpectExit()
	cypher.Expect(`
...
`)
	cypher.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	cypher := runCph(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	cypher := runCph(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer cypher.ExpectExit()
	cypher.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(cypher.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(cypher.StderrText(), m) {
...
// https://github.com/cypherium/cypherBFT-P/go-cypherium/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	cypher.SetTemplateFunc("keypath", func(file string) string {
