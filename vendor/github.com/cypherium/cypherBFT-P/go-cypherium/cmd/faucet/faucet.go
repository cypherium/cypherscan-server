...
import (
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/cph"
	"github.com/cypherium/cypherBFT-P/go-cypherium/cph/downloader"
	"github.com/cypherium/cypherBFT-P/go-cypherium/cphclient"
	"github.com/cypherium/cypherBFT-P/go-cypherium/cphstats"
	"github.com/cypherium/cypherBFT-P/go-cypherium/les"
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/p2p/discv5"
	"github.com/cypherium/cypherBFT-P/go-cypherium/p2p/nat"
...
var (
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Cypherium and the faucet")
)

var (
	cpher = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Cphers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	// Convert the bootnodes to internal cnode representations
	...
	faucet, err := newFaucet(genesis, *cphPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Cypherium address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Cypherium light client.
type faucet struct {
...
// close terminates the Cypherium connection and tears down the faucet.
...
// apiHandler handles requests for Cpher grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, cpher),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Cypherium address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/cypherium/cypherBFT-P/go-cypherium/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), cpher)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, cpher)
...
// the username, avatar URL and Cypherium address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Cypherium address and profile URL.
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Cypherium address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Cypherium address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Cypherium address,
