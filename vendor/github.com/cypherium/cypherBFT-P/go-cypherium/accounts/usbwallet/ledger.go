...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-cph/master/doc/ethapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Cypherium address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Cypherium transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Cypherium app in browser mode", w.failure
	}
	if w.offline() {
		return "Cypherium app offline", w.failure
	}
	return fmt.Sprintf("Cypherium app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Cypherium app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Cypherium app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Cypherium app's version, will fail prior to v1.0.2
...
// and returning the Cypherium address located on that derivation path.
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Cypherium app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Cypherium wallet app running
...
// ledgerDerive retrieves the currently active Cypherium address from a Ledger
...
//   Cypherium address length | 1 byte
//   Cypherium address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Cypherium hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Cypherium address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Cypherium signature and do a sanity validation
	if len(reply) != 65 {
