...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Cypherium
...
type wallet struct {
	...
	deriveChain    cypherI.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Cypherium account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain cypherI.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
func (w *wallet) GetKeyPair(account accounts.Account, passphrase string) ([]byte, []byte, error) {
