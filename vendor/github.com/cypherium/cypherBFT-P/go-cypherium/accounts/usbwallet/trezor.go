...
import (
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/accounts/usbwallet/internal/trezor"
...
// or whether the Cypherium app was not started on it.
...
// Cypherium address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.CphereumAddress)
	if _, err := w.trezorExchange(&trezor.CphereumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.CphereumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.CphereumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
