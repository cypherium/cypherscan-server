...
// Package filters implements an cypherium filtering system for block,
...
const (
	...
	// LatestTPSSubscription queries for latest TPS
	LatestTPSSubscription
...
const (
	...
	// tpsEvChanSize is the size of channel listening to latest TPS.
	tpsEvChanSize = 4
)
...
type EventSystem struct {
	...
	tpsSub          event.Subscription // Subscription for latest TPS
	...
	tpsCh          chan uint64                 // Channel to receive latest TPS
}
...
func NewEventSystem(mux *event.TypeMux, backend Backend, lightMode bool) *EventSystem {
	m := &EventSystem{
		...
		tpsCh:          make(chan uint64, tpsEvChanSize),
	}
	...
	m.tpsSub = m.backend.SubscribeLatestTPSEvent(m.tpsCh)
...
func (es *EventSystem) SubscribeLogs(crit cypherI.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit cypherI.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit cypherI.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit cypherI.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
// SubscribeLatestTPS creates a subscription that writes the latest TPS
func (es *EventSystem) SubscribeLatestTPS(tps chan uint64) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
		typ:       LatestTPSSubscription,
		created:   time.Now(),
		logs:      make(chan []*types.Log),
		hashes:    make(chan []common.Hash),
		tps:       tps,
...
func (es *EventSystem) broadcast(filters filterIndex, ev interface{}) {
	...
	case uint64:
		//log.Debug("Filter loop got latest TPS", "tps", e)
		for _, f := range filters[LatestTPSSubscription] {
			f.tps <- e
		}
	}
}
