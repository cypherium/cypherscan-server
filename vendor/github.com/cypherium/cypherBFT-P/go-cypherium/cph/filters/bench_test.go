...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/cypher/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := cphdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = cphdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db cphdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*cphdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db cphdb.Database) {
