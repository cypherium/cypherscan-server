...
type FakePeer struct {
	...
	db  cphdb.Database
	hc  *core.HeaderChain
	khc *core.KeyHeaderChain
	kbc *core.KeyBlockChain
	dl  *Downloader
}

// NewFakePeer creates a new mock downloader peer with the given data sources.
func NewFakePeer(id string, db cphdb.Database, hc *core.HeaderChain, khc *core.KeyHeaderChain, kbc *core.KeyBlockChain, dl *Downloader) *FakePeer {
	return &FakePeer{id: id, db: db, hc: hc, khc: khc, kbc: kbc, dl: dl}
}

// Head implements downloader.Peer, returning the current head hash and number
// of the best known header.
func (p *FakePeer) Head() (keyHeadHash common.Hash, keyHeadTd *big.Int, txHeadHash common.Hash, txHeight *big.Int) {

	keyHeader := p.khc.CurrentHeader()
	header := p.hc.CurrentHeader()

	return keyHeader.Hash(), keyHeader.Number, header.Hash(), new(big.Int).Set(header.Number)
}
...
func (p *FakePeer) RequestContinuousKeyBlocks(originNumber uint64, amount int) error {
	blocks := make([]*types.KeyBlock, 0)
	first := true
	next := originNumber
	var block *types.KeyBlock

	for len(blocks) < amount {
		if first {
			first = false

			block = p.kbc.GetBlockByNumber(originNumber)
			if block == nil {
				break
			}
			...
			block = p.kbc.GetBlockByNumber(next)
			next = next + 1
		}
	...
	p.dl.DeliverKeyBlocks(p.id, blocks)
	return nil
}
