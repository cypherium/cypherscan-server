...
func New(mode SyncMode, stateDb cphdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, keyChain KeyBlockChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
func (d *Downloader) Progress() cypherI.SyncProgress {
	...
	return cypherI.SyncProgress{
		...
		CurrentTxBlock:  current,
		HighestTxBlock:  d.syncStatsChainHeight,
		PulledTxStates:  d.syncStatsState.processed,
		KnownTxStates:   d.syncStatsState.processed + d.syncStatsState.pending,
	}
}
