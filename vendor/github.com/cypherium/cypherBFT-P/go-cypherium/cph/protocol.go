...
type getKeyBlocksData struct {
	Origin uint64 // Key block number from which to retrieve
	Amount uint64 // Maximum amount of key blocks to retrieve
}
...
// newKeyBlockData is the network packet for the key block propagation message.
type newKeyBlockData struct {
