...
import (
	...
	"path/filepath"
	"strings"
	//	clog "github.com/cypherium/cypherBFT-P/go-cypherium/log"
)
...
func (this *DirectoryClassPathEntry) readClass(className string) ([]byte, error) {
	//clog.Info("readClass", "className",className)
...
func NewClassPath(classPathStr string) *ClassPath {
	segs := strings.Split(classPathStr, "|") //old: segs := strings.Split(classPathStr, ":") //modify by kabel:
	// filter empty string
	var entries []string
...
func (this *ClassPath) String() string {
	...
	return strings.Join(entries, "|") //old: ":"
}
