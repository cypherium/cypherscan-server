...
func (n *TreeNodeInstance) RegisterChannelLength(c interface{}, length int) error {
	...
	log.Debug("Registered channel", "typ", typ, "with flags", flags)
	return nil
}
...
func (n *TreeNodeInstance) RegisterChannels(channels ...interface{}) error {
	for _, ch := range channels {
		if err := n.RegisterChannel(ch); err != nil {
			return fmt.Errorf("Error, could not register channel %T: %s", ch, err.Error())
		}
	}
	return nil
}
...
func (n *TreeNodeInstance) RegisterHandler(c interface{}) error {
	...
	n.handlers[typ] = c
	n.messageTypeFlags[typ] = flags
	log.Debug("Registered handler", "typ", typ, "with flags", flags)
	return nil
}
...
func (n *TreeNodeInstance) RegisterHandlers(handlers ...interface{}) error {
	for _, h := range handlers {
		if err := n.RegisterHandler(h); err != nil {
			return fmt.Errorf("Error, could not register handler %T: %s", h, err.Error())
		}
	}
	return nil
}
...
func (n *TreeNodeInstance) dispatchHandler(msgSlice []*ProtocolMsg) error {
	...
	if n.hasFlag(mt, AggregateMessages) {
		...
		log.Debug("Dispatching aggregation to", "", n.ServerIdentity().Address)
		errV = f.Call([]reflect.Value{msgs})[0]
	} else {
		for _, msg := range msgSlice {
			if errV.IsValid() && !errV.IsNil() {
				// Before overwriting an error, print it out
				log.Error("dispatchHandler", "name", n.Name(), "msg", reflect.TypeOf(msg.Msg), "error", errV.Interface().(error))
			}
			log.Debug("Dispatching", "msg", msg, "to", n.ServerIdentity().Address)
			m := n.reflectCreate(to, msg)
			errV = f.Call([]reflect.Value{m})[0]
		}
	}
	log.Debug("dispatchHandler", fmt.Sprintf("%s Done with handler for %s", n.Name(), f.Type()))
	if !errV.IsNil() {
...
func (n *TreeNodeInstance) dispatchChannel(msgSlice []*ProtocolMsg) error {
	...
	defer func() {
		...
		if r := recover(); r != nil {
			log.Error("dispatchChannel", fmt.Sprintf("Shouldn't happen, please report an issue:", n.Info(), r, mt))
		}
	}()
	to := reflect.TypeOf(n.channels[mt])
	if n.hasFlag(mt, AggregateMessages) {
		log.Debug("dispatchChannel", "Received aggregated message of type:", mt)
		to = to.Elem()
		out := reflect.MakeSlice(to, len(msgSlice), len(msgSlice))
		for i, msg := range msgSlice {
			log.Debug("dispatchChannel", "Dispatching aggregated to", to)
			m := n.reflectCreate(to.Elem(), msg)
			log.Debug("dispatchChannel", "Adding msg", m, "to", n.ServerIdentity().Address)
			out.Index(i).Set(m)
		}
		...
		for _, msg := range msgSlice {
			...
			m := n.reflectCreate(to.Elem(), msg)
			log.Debug("dispatchChannel", "name", n.Name(), "Dispatching msg type", mt, " to", to, " ", m.Field(1).Interface())
			if out.Len() < out.Cap() {
				n.msgDispatchQueueMutex.Lock()
				closing := n.closing
				n.msgDispatchQueueMutex.Unlock()
				...
				return fmt.Errorf("channel too small for msg %s in %s: "+"please use RegisterChannelLength()", mt, n.ProtocolName())
			}
		}
	}
	return nil
...
func (n *TreeNodeInstance) dispatchMsgReader() {
	for {
		...
		if len(n.msgDispatchQueue) > 0 {
			log.Debug("dispatchMsgReader", "info", n.Info(), "Read message and dispatching it", len(n.msgDispatchQueue))
			msg := n.msgDispatchQueue[0]
			n.msgDispatchQueue = n.msgDispatchQueue[1:]
			n.msgDispatchQueueMutex.Unlock()
			err := n.dispatchMsgToProtocol(msg)
			if err != nil {
				log.Error("dispatchMsgReader", "name", n.Name(), "msg", reflect.TypeOf(msg.Msg), "err", err)
			}
		} else {
			n.msgDispatchQueueMutex.Unlock()
			log.Debug("dispatchMsgReader", "info", n.Info(), "", "Waiting for message")
...
func (n *TreeNodeInstance) dispatchMsgToProtocol(onetMsg *ProtocolMsg) error {
	...
	if !done {
		log.Debug("Not done aggregating children msgs", "name", n.Name())
		return nil
	}
	log.Debug(fmt.Sprintf("%s->%s: Message is: %+v", onetMsg.From, n.Name(), onetMsg.Msg))

	var err error
	switch {
	case n.channels[msgType] != nil:
		log.Debug("", "Dispatching to channel", n.Name())
		err = n.dispatchChannel(msgs)
	case n.handlers[msgType] != nil:
		log.Debug("Dispatching to handler", "", n.ServerIdentity().Address)
...
func (n *TreeNodeInstance) aggregate(onetMsg *ProtocolMsg) (network.MessageTypeID, []*ProtocolMsg, bool) {
	...
	n.msgQueue[mt] = msgs
	log.Debug("aggregate", "address", n.ServerIdentity().Address, "received", len(msgs), "of", len(n.Children()))
