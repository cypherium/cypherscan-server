...
import (
	...
	"github.com/cypherium/cypherBFT-P/go-cypherium/onet/network"
...
func (o *Overlay) Process(env *network.Envelope) {
	...
	if err != nil {
		log.Error("Process", "unwrapping", err)
		return
	}
	switch true {
	...
	default:
		...
		if err != nil {
			log.Error("Process", "Msg Type", protoMsg.MsgType, "from", protoMsg.ServerIdentity, "produced error", err)
		}
	}
}
...
func (o *Overlay) TransmitMsg(onetMsg *ProtocolMsg, io MessageProxy) error {
	...
	if done {
		log.Warn("Message for TreeNodeInstance that is already finished")
		return nil
	}
	// if the TreeNodeInstance is not there, creates it
	if !ok {
		log.Debug("TransmitMsg", "Creating TreeNodeInstance at", o.server.ServerIdentity, "", onetMsg.To.ID())
		...
		log.Debug("Overlay.TransmitMsg go pi.Dispatch()")
		go pi.Dispatch()
		if err := o.RegisterProtocolInstance(pi); err != nil {
			return errors.New("Error Binding TreeNodeInstance and ProtocolInstance:" +
				err.Error())
		}
		log.Debug("TransmitMsg", "Address", o.server.Address(), "Overlay created new ProtocolInstace msg => ", fmt.Sprintf("%+v", onetMsg.To))
	}
	...
	go func() {
		...
		for _, pending := range o.pendingMsg {
			if t.ID.Equal(pending.ProtocolMsg.To.TreeID) {
				...
				if err != nil {
					log.Error("checkPendingMessages", "TransmitMsg failed:", err)
					continue
				}
			} else {
...
func (o *Overlay) requestTree(si *network.ServerIdentity, onetMsg *ProtocolMsg, io MessageProxy) error {
	...
	_, err = o.server.Send(si, msg, false)
	return err
}
...
func (o *Overlay) handleRequestTree(si *network.ServerIdentity, req *RequestTree, io MessageProxy) {
	...
	if err != nil {
		log.Error("handleRequestTree", "couldn't wrap TreeMarshal:", err)
		return
	}

	_, err = o.server.Send(si, msg, false)
	if err != nil {
		log.Error("handleRequestTree", "Couldn't send tree:", err)
	}
}

func (o *Overlay) handleSendTree(si *network.ServerIdentity, tm *TreeMarshal, io MessageProxy) {
	if tm.TreeID.IsNil() {
		log.Error("handleSendTree", "Received an empty Tree")
		return
	}
	...
	if roster == nil {
		...
		if err != nil {
			log.Error("handleSendTree", "could not wrap RequestRoster:", err)
		}
		if _, err := o.server.Send(si, msg, false); err != nil {
			log.Error("handleSendTree", "Requesting Roster in SendTree failed", err)
		}
	...
	if err != nil {
		log.Error("handleSendTree", "Couldn't create tree:", err)
		return
	}
	log.Debug("Received new tree")
	o.RegisterTree(tree)
}
...
func (o *Overlay) handleRequestRoster(si *network.ServerIdentity, req *RequestRoster, io MessageProxy) {
	...
	if roster == nil {
		...
		log.Warn("Requested entityList that we don't have")
		roster = &Roster{}
	}
	...
	if err != nil {
		log.Error("handleRequestRoster", "error wraping up roster:", err)
		return
	}

	_, err = o.server.Send(si, msg, false)
	if err != nil {
		log.Error("handleRequestRoster", "Couldn't send empty entity list from host:", o.server.ServerIdentity.String(), "error", err)
		return
	}
}
...
func (o *Overlay) SendToTreeNode(from *Token, to *TreeNode, msg network.Message, io MessageProxy, c *GenericConfig) (uint64, error) {
	...
	if c != nil {
		sentLen, err := o.server.Send(to.ServerIdentity, &ConfigMsg{*c, tokenTo.ID()}, false)
		totSentLen += sentLen
		if err != nil {
			log.Error("SendToTreeNode", "sending config failed:", err)
	...
	sentLen, err := o.server.Send(to.ServerIdentity, final, false)
	totSentLen += sentLen
	return totSentLen, err
}
...
func (o *Overlay) nodeDelete(token *Token) {
	...
	if !ok {
		log.Warn("nodeDelete", "Node already gone", tok)
		return
	}
	log.Debug("nodeDelete", "Closing node", tok)
	err := tni.closeDispatch()
	if err != nil {
		log.Error("nodeDelete", "Error while closing node:", err)
	}
...
func (o *Overlay) Close() {
	...
	for _, tni := range o.instances {
		log.Debug("Close", "address", o.server.Address(), "Closing TNI", tni.TokenID())
...
func (o *Overlay) CreateProtocol(name string, t *Tree, sid ServiceID) (ProtocolInstance, error) {
	...
	go func() {
		log.Debug("Overlay.CreateProtocol go pi.Dispatch()")
		err := pi.Dispatch()
		if err != nil {
			log.Error("CreateProtocol", "error", err)
		}
	}()
	return pi, err
}
...
func (o *Overlay) StartProtocol(name string, t *Tree, sid ServiceID) (ProtocolInstance, error) {
	...
	go func() {
		err := pi.Start()
		if err != nil {
			log.Error("StartProtocol", "Error while starting:", err)
		}
	}()
	return pi, err
}
...
func (o *Overlay) RegisterProtocolInstance(pi ProtocolInstance) error {
	...
	log.Debug("RegisterProtocolInstance", "", o.server.Address(), "registered ProtocolInstance", tok.ID())
	return nil
}
