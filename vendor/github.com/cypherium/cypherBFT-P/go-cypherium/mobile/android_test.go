...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Cypher.LightScryptN, Cypher.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.cypherium", new NodeConfig());
			node.start();
			...
			CphereumClient ec = node.getCphereumClient();
	...
	// signatures too. Regression test for cypherBFT issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "cypher-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/cypherium/gcphtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.cypherium", "github.com/cypherium/cypherBFT-P/go-cypherium/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "cypher.aar"), "cypher.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "cypherium", "gcphtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.interface.gcphtest"
