...
func newServerPool(db cphdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
...
func (pool *serverPool) connect(p *peer, ip net.IP, port uint16) *poolEntry {
	log.Debug("Connect new entry", "cnode", p.id)
...
func (pool *serverPool) registered(entry *poolEntry) {
	log.Debug("Registered new entry", "cnode", entry.id)
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	log.Debug("Disconnected old entry", "cnode", entry.id)
