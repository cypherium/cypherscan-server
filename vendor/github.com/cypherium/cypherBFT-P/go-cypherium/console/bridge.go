...
type bridge struct {
	client   *rpc.Client  // RPC client to execute Cypherium requests through
...
// StartMiner is a wrapper around the miner.start RPC method that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC method (saved in jeth.startMiner) with it to actually execute
// the RPC call.
func (b *bridge) StartMiner(call otto.FunctionCall) (response otto.Value) {
	// Make sure we have an account specified to unlock
	if !call.Argument(0).IsNumber() {
		throwJSException("first argument must be the thread number to run mining")
	}
	threadNumber := call.Argument(0)

	if !call.Argument(1).IsString() {
		throwJSException("second argument must be the account to be used as miner")
	}
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jeth.startMiner", nil, threadNumber, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("cph.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
