...
// backing transaction.
//
// AddUncheckedReceipt will cause pow failures when used during real
...
func makeHeader(chain types.ChainReader, parent *types.Block, state *state.StateDB, engine pow.Engine) *types.Header {
	var time *big.Int
	if parent.Time() == nil {
		time = big.NewInt(10)
	} else {
