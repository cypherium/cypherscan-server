...
func TestEIP155Signing(t *testing.T) {
	...
	publicByte, _ := kp.Public.MarshalBinary()
	privateByte, _ := kp.Private.MarshalBinary()

	addr := crypto.PubKeyToAddressCypherium(publicByte)

	signer := NewEIP155Signer(big.NewInt(18))
	tx, err := SignTxWithED25519(NewTransaction(0, addr, new(big.Int), 0, new(big.Int), nil), signer, privateByte, publicByte)
	if err != nil {
		t.Fatal(err)
	}
...
func TestChainId(t *testing.T) {
	...
	var err error
	tx, err = SignTxWithED25519(tx, NewEIP155Signer(big.NewInt(1)), privateByte, publicByte)
	if err != nil {
		t.Fatal(err)
	}
