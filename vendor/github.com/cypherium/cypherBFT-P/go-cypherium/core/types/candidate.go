...
func NewCandidate(parentHash common.Hash, difficulty *big.Int, number uint64, txNumber uint64, extraByte []byte, ip []byte, pubKey string, eb string, port int) *Candidate {
	c := &Candidate{
		KeyCandidate: &KeyBlockHeader{Version: KeyBlockVersion, BlockType: PowReconfig, ParentHash: parentHash, Difficulty: big.NewInt(0)},
		Port:         port,
	}
	c.KeyCandidate.Time = new(big.Int).SetInt64(time.Now().Unix())

	if len(extraByte) > 0 {
		c.KeyCandidate.Extra = make([]byte, len(extraByte))
		copy(c.KeyCandidate.Extra, extraByte)
	}
...
func (c *Candidate) HashNoNonce() common.Hash {
	keyBlockHeader := &KeyBlockHeader{
		Version:       c.KeyCandidate.Version,
