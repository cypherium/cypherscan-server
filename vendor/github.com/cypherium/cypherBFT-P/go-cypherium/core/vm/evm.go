...
func run(evm *EVM, contract *Contract, input []byte) ([]byte, error) {
	if contract.CodeAddr != nil {
		...
		//fmt.Println("precompiles not support!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
		//return nil, fmt.Errorf("precompiles not support!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
type Context struct {
	...
	// sufficient cpher to transfer the value
	CanTransfer CanTransferFunc
	// Transfer transfers cpher from one account to the other
...
// EVM is the Cypherium Virtual Machine base object and provides
...
type EVM struct {
	...
	// evm.
	vmConfig Config
	// global (to this context) cypherium virtual machine
...
func NewEVM(ctx Context, statedb StateDB, chainConfig *params.ChainConfig, vmConfig Config, chain types.ChainReader) *EVM {
	evm := &EVM{
		Context:      ctx,
		StateDB:      statedb,
		vmConfig:     vmConfig,
	...
	evm.interpreters[1] = NewEVMInterpreter(evm, vmConfig)
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	if params.DisableEVM {
		return nil, gas, ErrNoCompatibleInterpreter
	}
	if evm.vmConfig.NoRecursion && evm.depth > 0 {
		return nil, gas, nil
	}
...
func (evm *EVM) create(caller ContractRef, code []byte, gas uint64, value *big.Int, address common.Address) ([]byte, common.Address, uint64, error) {
	if params.DisableEVM {
		return nil, common.Address{}, gas, ErrNoCompatibleInterpreter
	}
