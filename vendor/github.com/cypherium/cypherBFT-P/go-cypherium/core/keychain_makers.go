...
func (b *KeyBlockGen) OffsetTime(seconds int64) {
	...
	b.header.Difficulty = b.engine.CalcKeyBlockDifficulty(b.keyChainReader, b.header.Time.Uint64(), b.parent.Header())
}
...
func GenerateKeyChain(config *params.ChainConfig, parent *types.KeyBlock, engine pow.Engine, db cphdb.Database, n int, gen func(int, *KeyBlockGen)) []*types.KeyBlock {
	...
	genKeyBlock := func(i int, parent *types.KeyBlock) *types.KeyBlock {
		...
		if b.engine != nil {
			...
			return block.WithSignatrue([]byte("I am signatrue"), nil)
			//return block
		}
		return nil
	}
...
func makeKeyHeader(chain types.KeyChainReader, parent *types.KeyBlock, engine pow.Engine) *types.KeyBlockHeader {
	var time *big.Int
	if parent.Time() == nil {
		time = big.NewInt(10)
	} else {
	...
	return &types.KeyBlockHeader{
		...
		Difficulty: engine.CalcKeyBlockDifficulty(chain, time.Uint64(), &types.KeyBlockHeader{
